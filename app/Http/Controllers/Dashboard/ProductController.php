...
class ProductController extends Controller
{
    ...
    {
        $product->orders->load('order.user');
        
        return view('admin.product.orders',compact('product'));
    }
    public function storeAttribute(Request $request, Product $product)
    {
       $product->productAttributes()->create(['name'=>$request->name,'quantity'=> $request->quantity]);
       $quantity = $product->quantity+$request->quantity;
       $product->update(['quantity'=> $quantity]);
       return back()->with('success','Attribute Addedd Successfully');
    }
    public function editAttribute(ProductAttribute $productAttribute)
    {
        return view('admin.product.attribute',compact('productAttribute'));
    }
    public function updateAttribute(Request $request, ProductAttribute $productAttribute)
    {
        ...
        $quantity = $productAttribute->quantity - $request->quantity;
        // dd($quantity);
        if($quantity <= 0){
           
             $productAttribute->product->increment('quantity',-$quantity);
        }
       
        elseif($quantity >= 0)
        {
           
            $productAttribute->product->decrement('quantity', $quantity);
        }
            
        
        $productAttribute->update($input);
